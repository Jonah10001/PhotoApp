...
from my_decorators import secure_bookmark, handle_db_insert_error, check_ownership_of_bookmark, is_valid_int
...
class BookmarksListEndpoint(Resource):
    ...
    def post(self):
        ...
        if Bookmark.query.filter_by(post_id=post_id, user_id=self.current_user.id).first():
            return Response(json.dumps({'message': 'No duplicates!'}), mimetype="application/json", status=400)
...
class BookmarkDetailEndpoint(Resource):
    ...
    def delete(self, id):
        ...
        try:
            ...
            return Response(json.dumps({'message': 'Not a valid ID entry'}), mimetype="application/json", status=400)
        ...
        if bookmark is None:
            return Response(json.dumps({'message': 'Not a valid ID entry'}), mimetype="application/json", status=404)
        elif bookmark.user_id is not self.current_user.id:
            return Response(json.dumps({'message': 'User does not have access to this bookmark'}), mimetype="application/json", status=404)
